<template>
  <div>
    ...
    <!-- fullName here is a method call in scripts -->
    <h2>
      Total with normal calculate -
    ...
    <h1>Using v-for</h1>
    <!-- If using template to show id that will make error -->
    <h2>Item in Items</h2>
    <div v-for="item in items" :key="item.id">
      <h4 v-if="item.id === 1">{{ item.title }} - {{ item.price }}</h4>
    </div>
    <!--  -->
    <h2>Item in expensiveItems</h2>
    <h4 v-for="item in expensiveItems" :key="item.id">
    ...
    <button @click="movie_list = movie_list.concat(['Wonder Woman'])">
      Add movie
    </button>
  </div>
</template>
...
export default {
  ...
  computed: {
    // if you want to make get set you have to make it become objects
    // by using { }
    fullName: {
      get() {
    ...
    volume: {
      handler(newValue, oldValue) {
        ...
        if (newValue > oldValue && newValue === 16) {
          // chỉ cảnh cáo 1 lần tương tự như trong điện thoại
          alert(
